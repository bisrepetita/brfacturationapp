...
import { getInvoice, updateDraftInvoice, validateInvoice } from "@/lib/invoices";
...
export default function ModifierPage() {
  ...
  // Charger la facture et pré-remplir
  useEffect(() => {
    getInvoice(id).then(async (invoice) => {
  ...
  const handleSaveDraft = async () => {
    ...
    setError("");
    try {
      await updateDraftInvoice(id, buildUpdateData(), user.uid);
      router.push(`/factures/${id}`);
    } catch (e) {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#F9F8F6" }}>
      {/* Header */}
      <div className="sticky top-0 z-10 flex items-center gap-3 px-4 h-14 border-b" style={{ backgroundColor: "#FFFFFF", borderColor: "#E5E1DA" }}>
        <button
          onClick={() => step === 1 ? router.push(`/factures/${id}`) : setStep((s) => (s - 1) as WizardStep)}
      ...
      {step < 4 && (
        ...
        >
          ...
          <button
            ...
            disabled={!canGoNext()}
            className="w-full h-12 rounded-[8px] text-[14px] font-medium text-white flex items-center justify-center gap-2 disabled:opacity-40"
        ...
        <div className="space-y-2">
          {filtered.map((client) => (
            <button
              ...
              className="w-full rounded-[10px] border p-4 flex items-center gap-3 text-left"
              style={{
...
function Step2Lines({
  ...
  const [showMultiDate, setShowMultiDate] = useState<Service | null>(null);
  ...
  const addLine = useCallback(
    (patch: Partial<InvoiceLine>) => {
      ...
      onChange([...lines, { ...base, ...calculateLine(base) }]);
    },
    [lines, onChange]
  );

  const updateLine = useCallback(
    (lineId: string, patch: Partial<InvoiceLine>) => {
      onChange(
        lines.map((l) => {
          if (l.id !== lineId) return l;
          const u = { ...l, ...patch };
          return { ...u, ...calculateLine(u) };
        })
      );
    },
    [lines, onChange]
            ...
            {filteredServices.map((s) => (
              <div key={s.id} className="flex items-center gap-2">
                <button
                  ...
                  className="flex-1 text-left px-3 py-2.5 rounded-[8px] text-[14px]"
                  style={{ color: "#1A1A18" }}
...
function LineCard({
  ...
  const inputStyle = { backgroundColor: "#F9F8F6", borderColor: "#E5E1DA", color: "#1A1A18" };
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                {line.vatApplicable && (
                  <select
                    ...
                    onChange={(e) =>
                      onUpdate(line.id, { vatRate: e.target.value ? (parseFloat(e.target.value) as VatRate) : null })
                    }
...
function MultiDatePicker({
  ...
  service: Service;
  onConfirm: (s: Service, dates: Date[]) => void;
  onClose: () => void;
}) {
  ...
  const monthStr = new Date(viewYear, viewMonth).toLocaleDateString("fr-CH", {
    month: "long",
    year: "numeric",
  });
  ...
  return (
    <ModalOverlay onClose={onClose}>
      ...
      <p className="text-[13px] mb-4" style={{ color: "#7A7570" }}>
        Sélectionnez plusieurs dates pour créer une ligne par date.
      </p>
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <button
              ...
              className="aspect-square rounded-[6px] text-[13px] font-medium"
              style={{ backgroundColor: sel ? "#1A1A18" : "transparent", color: sel ? "#FFFFFF" : "#1A1A18" }}
              onMouseEnter={(e) => { if (!sel) e.currentTarget.style.backgroundColor = "#F0EDE8"; }}
      ...
      <button
        onClick={() => onConfirm(service, selectedDates.map((d) => new Date(d + "T12:00:00")))}
...
function Step3Conditions({
  ...
  const fi = (e: React.FocusEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-[8px] px-4 py-3 text-[13px]" style={{ backgroundColor: "#F0EDE8", color: "#7A7570" }}>
        ...
        <div className="p-5 border-b" style={{ borderColor: "#E5E1DA" }}>
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              <p className="text-[12px]" style={{ color: "#7A7570" }}>{format(invoiceDate, "dd.MM.yyyy", { locale: fr })}</p>
              <p className="text-[12px]" style={{ color: "#7A7570" }}>Éch. {format(dueDate, "dd.MM.yyyy", { locale: fr })}</p>
            </div>
          </div>
        </div>
